"""PPIFlow-Pipeline source repo: <https://github.com/zhuqianhui2-hash/PPIFlow-Pipeline>.
...
| `--framework-pdb` | **Required** | Path to the VHH framework PDB file used as the design scaffold. CDR positions must be labeled using IMGT numbering, and the CDR loops residues should be removed. |
...
| `--num-devices` | `all` | Number of GPU workers for GPU-bound steps (e.g. `2`, or `all` for every GPU). Users can customize.|
| `--num-rosetta-workers` | `None` | CPU worker pool size for Rosetta steps. Defaults to GPU pool size. |
...
* <https://github.com/zhuqianhui2-hash/PPIFlow-Pipeline/blob/main/src/pipeline/cli.py>
* <https://github.com/zhuqianhui2-hash/PPIFlow-Pipeline/blob/main/documentation/cli_reference.md>

| Environment variable | Default | Description |
|----------------------|---------|-------------|
| `MODAL_APP` | `PPIFlow` | Name of the Modal app to use. |
| `GPU` | `L40S:4` | GPU type and count for the Modal container. 4 GPUs balances throughput, cost, and preemption risk. Override with e.g. `GPU=L40S:8` for maximum parallelism (higher preemption risk). L40S supports max 8 per container. |
...
* Results are written to the persistent Modal volume under `/ppiflow-outputs/{run_name}/`.
* The output directory contains numbered step folders `step_01_gen` through `step_15_rank_finalize`, the native pipeline working
  directory in `_pipeline/`, archived input PDBs in `inputs/`, and a `run_summary.txt` execution summary.
* If the client disconnects, the remote Modal function continues running, and re-running with the same `--run-name` resumes from the last completed step.

## Multi-worker execution model

The 15 pipeline stages still run in sequence. Parallelism happens within each
individual stage rather than across stages.

| Topic | Default / Setting | Meaning |
|------|-------------------|---------|
| Overall flow | 15 stages, sequential | The pipeline advances stage by stage. Only the active stage can use workers in parallel. |
| Default layout | ``GPU=L40S:4`` | 1 Modal container with 4 GPUs.  Each GPU-parallel step spawns 4 workers (one per GPU).  Override via ``GPU=L40S:8`` for 8 workers at the cost of higher preemption risk. |
| GPU-parallel stages | ``gen``, ``flowpacker1``, ``af3score1``, ``partial``, ``flowpacker2``, ``af3score2``, ``af3_refold`` | These stages can run multiple workers in parallel. Each worker gets one Modal-allocated GPU, workers coordinate through a SQLite work queue, and each worker is pinned to its own ``CUDA_VISIBLE_DEVICES``. |
| CPU multi-worker stages | ``rosetta_interface``, ``relax`` | These stages run on CPU and use a configurable worker pool controlled by ``--num-rosetta-workers``. If unset, the default matches the GPU pool size. The ``relax`` step also produces post-relax interface energy scores (merged from the former ``rosetta_interface2`` step). |
| CPU serial / leader-only stages | ``seq1``, ``interface_enrich``, ``seq2``, ``rank_features``, ``rank_finalize`` | These stages are effectively single-process even if a larger worker pool is configured. |
| CPU scoring with possible parallel dispatch | ``dockq`` | DockQ itself is not a GPU algorithm, but the orchestrator may still dispatch it with a larger worker pool. |
| GPU worker count | ``--num-devices`` | Controls how many workers GPU-bound stages can run with. For example, ``--num-devices 8`` allows up to 8 workers for GPU-parallel stages when 8 GPUs are allocated. |
| CPU worker count | ``--num-rosetta-workers`` | Controls the worker count for Rosetta CPU stages independently of the GPU pool. |

## Pipeline step mapping

The script delegates to `ppiflow pipeline` (orchestrator mode) which runs the
official 15-step workflow:
...
| 11 | `relax` | Rosetta relaxation + post-relax per-residue contact energy (collected for ranking) |
| 12 | `af3_refold` | AF3 refolding validation (only processes af3score2-passed subset) |
| 13 | `dockq` | DockQ structural quality |
| 14 | `rank_features` | Collect ranking features |
| 15 | `rank_finalize` | Final ranked summary |

Steps 10 (af3score2) and 11 (relax) both read from step 9 (flowpacker2) output.
Relax no longer depends on af3score2 filtering. Step 12 (af3_refold) only
processes the intersection of relaxed PDBs that passed af3score2 filtering.

When `--skip-refold` is set, steps 12 (af3_refold) and 13 (dockq) are removed
and ranking uses AF3Score round-2 metrics.

### How `rosetta_interface` (step 5) and `relax` (step 11) work

Both steps use the same `native.xml` Rosetta protocol.
...
**Step 11 output** includes both relaxed PDBs and interface energy scores
(`residue_energy.csv`, `interface_scores.csv`). These scores are collected as
ranking features; the pipeline's final ranking is driven by AF3Score ipTM/pTM
and DockQ composite score, not by Rosetta energy alone.
"""
...
# --- GPU and timeout ---
# Default to 4 GPUs: good throughput with much lower preemption risk than 8.
# During CPU-only steps (rosetta, relax, seq, rank), GPUs sit idle but still
# cost money — fewer GPUs = less waste.  Override via GPU env var,
# e.g. GPU=L40S:8 modal run ...  L40S max is 8 per container.
# See https://modal.com/docs/guide/gpu
GPU = os.environ.get("GPU", "L40S:4")
...
# --- Repository pins ---
PPIFLOW_PIPELINE_REPO = "https://github.com/zhuqianhui2-hash/PPIFlow-Pipeline.git"
PPIFLOW_PIPELINE_COMMIT = "a71363e955906e4dfb8b12285fa33e78b243a7c5"
...
STEP_OUTPUT_MAP = [
    ...
    ("step_12_af3_refold", "output/af3_refold"),
    ("step_13_dockq", "output/dockq"),
    ("step_14_rank_features", "results/features"),
    ("step_15_rank_finalize", "results"),
]
...
runtime_image = (
    ...
    .run_commands(
        *_stub_commands(),
        ...
        # The pinned commit has the entrypoint at proofsrc/ppiflow.py but the
        # orchestrator's _build_worker_cmd() expects a root-level ppiflow.py.
        # Create a symlink so both paths work.
        f"ln -sf {PIPELINE_ROOT / 'proofsrc' / 'ppiflow.py'} {PIPELINE_ROOT / 'ppiflow.py'}",
...
def run_vhh_pipeline(
    ...
    num_devices: str = "all",
    num_rosetta_workers: int | None = None,
    extra_pipeline_args: str | None = None,
) -> dict:
    """Execute the full PPIFlow-Pipeline de novo VHH workflow in multi-worker mode.

    The upstream orchestrator spawns per-step worker pools inside this
    container.  GPU-bound steps (gen, flowpacker, af3score, partial,
    af3_refold) get one worker per visible GPU; CPU-bound steps
    (rosetta_interface, relax) use ``num_rosetta_workers`` workers.
    ...
    # Commit every 60 s to minimise data loss on preemption.  Modal may
    # preempt GPU containers at any time; a shorter interval means less
    # re-work when the pipeline resumes via --reuse.
    ...
    # ── 07. Build CLI for ``ppiflow pipeline`` (orchestrator mode) ────
    # NOTE: no --single-process here — the orchestrator manages worker
    # pools, CUDA_VISIBLE_DEVICES binding, and work-queue coordination.
    cmd: list[str] = [
    ...
    # ── 07a. Multi-worker / multi-GPU arguments ──────────────────────
    if num_devices:
        cmd.extend(["--num-devices", str(num_devices)])

    if num_rosetta_workers is not None:
        cmd.extend(["--num-rosetta-workers", str(num_rosetta_workers)])
    ...
    # ── 08. Execute the pipeline (orchestrator mode) ──────────────────
    print(f"▶ Starting VHH pipeline (multi-worker): run_name={run_name}")
    print(f"  output → {pipeline_dir}")
    if num_devices:
        print(f"  num-devices → {num_devices}")
    if num_rosetta_workers is not None:
        print(f"  rosetta-workers → {num_rosetta_workers}")
    pipeline_ok = True
    try:
    ...
    summary_lines = [
        ...
        f"mode: multi-worker (orchestrator)",
...
def main(
    ...
    skip_refold: bool = False,
    num_devices: str = "all",
    num_rosetta_workers: int = 0,
    extra_pipeline_args: str | None = None,
) -> None:
    """Run a de novo VHH design on Modal with multi-GPU / multi-worker support.

    This is the multi-worker counterpart of ``ppiflow_single_app.py``.
    Instead of forcing ``--single-process``, it delegates to the upstream
    PPIFlow-Pipeline orchestrator which spawns per-step worker pools,
    distributes GPU work across all visible GPUs, and coordinates via a
    work queue.

    Results are saved to the ``ppiflow-outputs`` volume at
    ``/ppiflow-outputs/{run_name}/``, organized into 15 step sub-directories.
    ...
    # -- Resolve optional int args (Modal CLI passes 0 for unset ints) --
    resolved_rosetta = num_rosetta_workers if num_rosetta_workers > 0 else None

    # -- Submit to Modal --
    print(f"🧬 Submitting VHH pipeline (multi-worker): run_name={run_name}")
    print(f"   GPU spec: {GPU}")
    if num_devices:
        print(f"   Num devices: {num_devices}")
    if resolved_rosetta is not None:
        print(f"   Rosetta workers: {resolved_rosetta}")
    ...
    result = run_vhh_pipeline.remote(
        ...
        num_rosetta_workers=resolved_rosetta,
